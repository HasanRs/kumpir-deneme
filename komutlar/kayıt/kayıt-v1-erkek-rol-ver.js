...
exports.run = (client, message, args) => {
    ...
    if (!rol) {
        message.reply('Lütfen erkek kayıtında alınacak rolü etiketleyin!');
        return;
    }
    let rol01 = db.get(`kayıt_üye.alınacak.${message.guild.id}`);
    let rol0 = db.get(`kayıt_üye.verilecek.${message.guild.id}`);
    let rol1 = db.get(`kayıt_kız.verilecek.${message.guild.id}`);
    let rol2 = db.get(`kayıt_erkek.alınacak.${message.guild.id}`);
    let rol3 = db.get(`kayıt_kız.alınacak.${message.guild.id}`);
    ...
    db.set(`kayıt_erkek.verilecek.${message.guild.id}`, rol.id);
...
exports.help = {
    name: 'erkek-rol-ver',
    description: 'Kayıt sisteminde verilecek erkek rolünü ayarlar.',
    usage: 'erkek-rol-ver @rol',
};
